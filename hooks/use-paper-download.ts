...
// 预估各阶段时间（秒）
const _PHASE_ESTIMATED_TIME: Record<DownloadPhase, number> = {
  idle: 0,
  searching: 2,
  downloading: 10,
  processing: 15,
...
export function usePaperDownload() {
  ...
  const [downloadMap, setDownloadMap] = useState<Map<string, DownloadStatus>>(
    new Map(),
  );
  ...
  const updateDownloadStatus = useCallback(
    (arxivId: string, updates: Partial<DownloadStatus>) => {
      setDownloadMap((prev) => {
  ...
  const setDownloadStatus = useCallback(
    (arxivId: string, status: DownloadStatus) => {
      setDownloadMap((prev) => {
        const newMap = new Map(prev);
  ...
  const getDownloadStatus = useCallback(
    (arxivId: string): DownloadStatus | undefined => {
    ...
    async (
      ...
      try {
        ...
        if (autoNavigate && result.document_id) {
          setTimeout(() => {
            router.push(
              `/paper-search?kb=${kbId}&highlight=${result.document_id}`,
            );
          }, 1500);
        }
    ...
    async (
      ...
      options?: {
        onComplete?: (results: {
          success: number;
          failed: number;
          results: Map<string, DownloadResult | null>;
        }) => void;
        autoRefresh?: boolean;
      },
    ) => {
      ...
      const downloadWithConcurrency = async () => {
        ...
        const processNext = async (): Promise<void> => {
          ...
          setBatchProgress((prev) =>
            prev ? { ...prev, inProgress: prev.inProgress + 1 } : null,
          );
      ...
      if (failed === 0) {
        ...
        toast.warning(
          t.batchDownloadPartial
            ?.replace("{success}", String(completed))
  ...
  return {
    ...
    status: Array.from(downloadMap.values()).find(
      (s) =>
        s.phase !== "idle" && s.phase !== "completed" && s.phase !== "error",
    ) || {
