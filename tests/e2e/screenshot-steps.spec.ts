...
test('登录并每步截图', async ({ page }) => {
  ...
  // 步骤6: 点击设置按钮
  const settingsButton = page.locator('button:has-text("设置"), [data-testid="settings-button"], button[aria-label*="设置"]').first();
  await settingsButton.click();
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'step-06-settings-dialog.png', fullPage: true });
  console.log('步骤6: 设置对话框');

  // 步骤7: 点击知识库标签
  const kbTab = page.locator('[role="tab"]:has-text("知识库"), button:has-text("知识库")').first();
  if (await kbTab.isVisible()) {
    await kbTab.click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'step-07-knowledge-base-tab.png', fullPage: true });
    console.log('步骤7: 知识库标签页');
  }

  // 步骤8: 展开知识库查看文档列表
  const expandButton = page.locator('button:has([class*="chevron"]), button:has-text("展开"), [data-state="closed"]').first();
