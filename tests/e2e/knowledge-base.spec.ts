...
async function login(page: Page) {
  ...
  try {
    ...
    await page.waitForSelector(
      'button:has-text("对话"), button:has-text("新对话")',
      { timeout: 15000 },
    );
  } catch (e) {
...
test.describe("Authentication", () => {
  test("should login successfully", async ({ page }) => {
    ...
    const sidebarIndicator = page
      .locator(
        'button:has-text("对话"), button:has-text("论文搜索"), button:has-text("新对话")',
      )
      .first();
  ...
  test("should logout successfully", async ({ page }) => {
    ...
    if (count > 1) {
      ...
      const logoutButton = page.locator(
        'button[aria-label*="logout"], button[aria-label*="退出"]',
      );
...
test.describe("Knowledge Base Management", () => {
  ...
  test("should delete a knowledge base", async ({ page }) => {
    ...
    const kbItem = page
      .locator('[data-testid="kb-item"], .knowledge-base-item')
      .first();
    if (await kbItem.isVisible()) {
      ...
      // Click delete
      const deleteButton = kbItem
        .locator(
          '[data-testid="delete-kb"], button:has-text("删除"), button:has-text("Delete")',
        )
        .first();
...
test.describe("Chat with Knowledge Base", () => {
  ...
  test("should select knowledge base for chat", async ({ page }) => {
    ...
    if (await kbItem.isVisible({ timeout: 2000 })) {
      ...
      const selectButton = kbItem.locator(
        'button:has-text("选择"), button:has-text("Select"), button:has-text("使用")',
      );
