...
const testResults: {
  test1: { success: boolean; resultCount: number; error?: string; inputType?: string };
  test2: { success: boolean; resultCount: number; error?: string; inputType?: string };
  test3: { success: boolean; resultCount: number; error?: string; inputType?: string };
} = {
...
test.describe('Intelligent Search Feature Tests', () => {
  ...
  test('Test 1: Short text (keyword search) - "transformer"', async ({ page }) => {
    ...
    await page.route('**', async (route) => {
      ...
      // Mock search API
      if (url.includes('/api/v1/papers/search') || url.includes('/api/v1/papers/smart-search')) {
        const urlObj = new URL(url, 'http://localhost');
        const query = urlObj.searchParams.get('query') || urlObj.searchParams.get('keyword') || '';
        searchMode = query.length < 20 ? 'keyword' : 'smart';
        
        console.log(`[Test 1] Search API called with query: "${query}" (mode: ${searchMode})`);
    ...
    await page.screenshot({ path: 'test-results/smart-search-test/01-initial-page.png', fullPage: true });
    
    // Verify page title
    const heading = page.getByRole('heading', { name: /论文搜索|Paper Search/ });
    ...
    await page.screenshot({ path: 'test-results/smart-search-test/02-short-text-input.png' });
    
    // Check if input type indicator is shown (should show "关键词搜索")
    const inputTypeIndicator = page.locator('text=/关键词搜索|Keyword Search|关键词|Keyword/i');
    const isInputTypeVisible = await inputTypeIndicator.isVisible().catch(() => false);
    console.log('[Test 1] Input type indicator visible:', isInputTypeVisible);
    
    // Click search button
    const searchButton = page.locator('button').filter({ has: page.locator('svg') }).nth(2);
    ...
    await page.screenshot({ path: 'test-results/smart-search-test/03-short-text-results.png', fullPage: true });
    
    // Check for results
    const paperTitle = page.locator('text=/Transformer|Attention/i');
    const hasResults = await paperTitle.isVisible({ timeout: 5000 }).catch(() => false);
    
    // Count results
    const paperCards = await page.locator('[class*="border"][class*="rounded-lg"]').count();
  ...
  test('Test 2: Medium text (natural language) - Chinese query', async ({ page }) => {
    test.setTimeout(60000);
              ...
              papers: [
                {
                  ...
                  abstract: 'This paper provides a comprehensive survey of deep learning methods for neural networks.',
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'test-results/smart-search-test/04-medium-text-input.png' });
    
    // Check if input type indicator shows "自然语言" or similar
    const inputTypeIndicator = page.locator('text=/自然语言|Natural Language|智能识别|Smart/i');
    const isInputTypeVisible = await inputTypeIndicator.isVisible().catch(() => false);
    ...
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'test-results/smart-search-test/05-medium-text-results.png', fullPage: true });
    
    // Check for results
    const paperTitle = page.locator('text=/Deep Learning|Neural Networks|深度学习/i');
    const hasResults = await paperTitle.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('Test 3: Long text (smart recognition) - Paper abstract', async ({ page }) => {
    test.setTimeout(60000);
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            data: {
              papers: [
                {
                  ...
                  abstract: 'This paper presents novel attention mechanisms for natural language processing tasks.',
              ...
              extracted_info: {
                detected_title: null,
                keywords: ['attention', 'deep neural networks', 'NLP', 'natural language processing'],
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'test-results/smart-search-test/06-long-text-input.png' });
    
    // Check if input type indicator shows "智能识别" or similar
    const inputTypeIndicator = page.locator('text=/智能识别|Smart Detection|智能/i');
    const isInputTypeVisible = await inputTypeIndicator.isVisible().catch(() => false);
    ...
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'test-results/smart-search-test/07-long-text-results.png', fullPage: true });
    ...
    const extractedInfoSection = page.locator('text=/识别结果|Extracted Info|关键词|Keywords/i');
    const hasExtractedInfo = await extractedInfoSection.isVisible().catch(() => false);
  ...
  test('Generate test report', async ({ page }) => {
    ...
    console.log(`   Result: ${testResults.test1.success ? '✅ PASS' : '❌ FAIL'}`);
    ...
    console.log(`   Result: ${testResults.test2.success ? '✅ PASS' : '❌ FAIL'}`);
    ...
    console.log(`   Result: ${testResults.test3.success ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`   Results found: ${testResults.test3.resultCount}`);
    console.log(`   Input type detected: ${testResults.test3.inputType}`);
    
    console.log('\n' + '='.repeat(60));
    const allPassed = testResults.test1.success && testResults.test2.success && testResults.test3.success;
...
`;
    ...
    const fs = require('fs');
    fs.writeFileSync('test-results/smart-search-test/TEST_REPORT.md', reportContent);
    console.log('Report saved to: test-results/smart-search-test/TEST_REPORT.md');
