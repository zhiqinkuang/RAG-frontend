...
const _savedAuthState: { cookies: any[]; origins: any[] } | null = null;
...
async function openPaperSearch(page: Page) {
  ...
  const paperSearchButton = page
    .locator('button:has-text("论文搜索"), button:has-text("Paper")')
    .first();
...
test.describe("Paper Search API Tests", () => {
  ...
  test("should get backup path via API", async ({ request }) => {
    ...
    const pathResponse = await request.get(
      `${API_URL}/api/v1/settings/paper-backup-path`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  test("should download paper via API", async ({ request }) => {
    ...
    if (kbData.data.knowledge_bases && kbData.data.knowledge_bases.length > 0) {
      ...
      const downloadResponse = await request.post(
        `${API_URL}/api/v1/papers/download`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
...
test.describe("Paper Search UI Tests", () => {
  ...
  test("should display paper search page", async ({ page }) => {
    ...
    const searchButton = page
      .locator('button:has-text("搜索"), button:has-text("Search")')
      .first();
  ...
  test("should search for papers", async ({ page }) => {
              ...
              papers: [
                {
                  ...
                  arxiv_id: "2301.00001",
                  title:
                    "Attention Is All You Need: A Comprehensive Survey on Transformer Models",
                  authors: ["Author One", "Author Two", "Author Three"],
                  abstract:
                    "This is a test abstract for the mock paper about transformer architectures and attention mechanisms in deep learning.",
    ...
    try {
      ...
      // 如果没有捕获到响应，继续检查结果
      console.log(
        "No response captured, checking if mock was triggered:",
        mockTriggered,
      );
  ...
  test("should download paper to knowledge base", async ({ page }) => {
    ...
    let _downloadMockTriggered = false;
    let optionsTriggered = false;
    const requestLog: string[] = [];
              ...
              papers: [
                {
                  ...
                  arxiv_id: "2201.00978",
                  title:
                    "PyramidTNT: Transformer in Transformer for Image Classification",
                  authors: ["Kai Han", "An Xiao", "Enhua Tan"],
                  abstract:
                    "This paper presents PyramidTNT, a novel architecture for image classification.",
    ...
    const downloadButton = page
      .locator('button:has-text("下载"), button:has-text("Download")')
      .first();
