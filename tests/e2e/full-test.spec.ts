...
test('完整测试文档筛选和浏览功能', async ({ page }) => {
  ...
  const docItem = page.locator('div.group:has(button[role="checkbox"])').first();
  ...
  if (await docItem.isVisible()) {
    ...
    if (await previewButton.isVisible()) {
      ...
      if (pages.length > 1) {
        ...
        await previewPage.screenshot({ path: 'pic-13-preview.png', fullPage: true });
    ...
    // 获取侧边栏文本内容
    const sidebarText = await page.locator('div.w-56').textContent().catch(() => '无法获取');
