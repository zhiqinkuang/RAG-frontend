...
test.describe('Security: New User Data Isolation', () => {
  ...
  test('Step 1: Register a new user', async () => {
    ...
    await page.waitForSelector('textarea, [data-sidebar], button', { timeout: 10000 }).catch(() => {});
  ...
  test('Step 2: Check knowledge base list is empty', async () => {
    ...
    await page.waitForSelector('[data-sidebar="footer"]', { timeout: 10000 }).catch(() => {});
    ...
    const settingsByIcon = page.locator('button:has(svg.lucide-settings)').first();
    ...
    if (!settingsButtonClicked) {
      ...
      if (count > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const hasSettingsIcon = await btn.locator('svg.lucide-settings').count() > 0;
          if (hasSettingsIcon) {
            await btn.click();
            settingsButtonClicked = true;
        ...
        if (!settingsButtonClicked && count >= 1) {
          ...
          const hasLogoutIcon = await firstBtn.locator('svg.lucide-log-out').count() > 0;
          if (!hasLogoutIcon) {
    ...
    if (!settingsButtonClicked) {
      const settingsByRole = page.getByRole('button', { name: /设置|settings/i }).first();
      if (await settingsByRole.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (!settingsButtonClicked) {
      ...
      for (let i = 0; i < Math.min(btnCount, 20); i++) {
        ...
        if (svgCount > 0) {
          ...
          const parent = await btn.evaluateHandle(el => el.closest('[data-sidebar]'));
          if (parent) {
            await btn.click();
            settingsButtonClicked = true;
    ...
    if (!settingsButtonClicked) {
      console.log('   ⚠️  Could not find settings button, taking screenshot for debugging');
      await page.screenshot({ path: 'test-results/security-debug-no-settings.png' });
...
test.describe('Security: Cross-user data access attempt', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    if (crossUserToken) {
      console.log(`   ✅ Cross-user token obtained: ${crossUserToken.substring(0, 20)}...`);
    } else {
  ...
  test('Attempt to access other user\'s KB by ID', async ({ request }) => {
    ...
    const myKBResponse = await request.get(`${API_URL}/api/v1/knowledge-bases`, {
    ...
    const kbList = myKBData.data?.knowledge_bases || myKBData.data?.items || (Array.isArray(myKBData.data) ? myKBData.data : []);
    const myKBIds = new Set(kbList.map((kb: any) => kb.id) || []);
    console.log(`   Current user's KB IDs: ${[...myKBIds].join(', ') || 'none'}`);
    ...
    for (const id of idsToTry) {
      ...
      if (kbResponse.status() === 200) {
        ...
        if (kbUserId && !myKBIds.has(kbData.data?.id)) {
          console.log(`   🚨 SECURITY ISSUE: Can access KB ${id} belonging to user ${kbUserId}`);
          securityIssuesFound++;
        } else {
