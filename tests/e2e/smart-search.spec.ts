...
test.describe('Smart Search Feature Tests', () => {
  ...
  test('should display paper search page with smart search UI', async ({ page }) => {
    await openPaperSearch(page);
    
    // Take screenshot of initial page
    await page.screenshot({ path: 'test-results/smart-search/01-initial-page.png', fullPage: true });
  ...
  test('should trigger keyword search for short text', async ({ page }) => {
    ...
    await page.route('**/api/v1/papers/search**', async (route) => {
      ...
      searchMode = query ? 'keyword' : 'unknown';
    ...
    await page.screenshot({ path: 'test-results/smart-search/02-short-text-input.png' });
    
    // Click search button (the button with search icon next to textbox)
    const searchButton = page.locator('button').filter({ has: page.locator('img') }).nth(2);
    ...
    await page.screenshot({ path: 'test-results/smart-search/03-short-text-results.png', fullPage: true });
    
    console.log('✅ Short text search completed');
  });

  test('should trigger natural language processing for medium text', async ({ page }) => {
    test.setTimeout(60000);
        ...
        body: JSON.stringify({
          ...
          data: {
            papers: [{
              ...
              abstract: 'A comprehensive study on deep learning methods for NLP...',
    ...
    await page.screenshot({ path: 'test-results/smart-search/04-medium-text-input.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/05-medium-text-results.png', fullPage: true });
    
    console.log('✅ Medium text search completed');
  });

  test('should trigger smart recognition for long text (abstract)', async ({ page }) => {
    test.setTimeout(60000);
    ...
    await page.screenshot({ path: 'test-results/smart-search/06-long-text-input.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/07-long-text-results.png', fullPage: true });
  ...
  test('should use hot search tags', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/smart-search/08-hot-tags.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/09-hot-tag-results.png', fullPage: true });
  ...
  test('should use suggestion prompts', async ({ page }) => {
    ...
    const suggestion = page.locator('button:has-text("帮我找关于深度学习图像分类的最新论文")');
    await expect(suggestion).toBeVisible({ timeout: 5000 });
    
    await page.screenshot({ path: 'test-results/smart-search/10-suggestions.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/11-suggestion-results.png', fullPage: true });
    
    console.log('✅ Suggestion prompt search completed');
  });

  test('should switch between smart search and keyword mode', async ({ page }) => {
    await openPaperSearch(page);
    ...
    await page.screenshot({ path: 'test-results/smart-search/12-mode-smart-active.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/13-mode-keyword-active.png' });
    ...
    await page.screenshot({ path: 'test-results/smart-search/14-mode-switch-complete.png' });
